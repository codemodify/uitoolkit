...
`replace`, no internal imports:

| what left | where it went | what it was here |
| --- | --- | --- |
| **Mail** — the daemon, the GUI client and the demo launcher | [codemodify/comms-mail](https://github.com/codemodify/comms-mail) | `examples/mail` (package `mailapp`), `cmd/mailclientd`, `cmd/mailclientui` |
| **The three players** — Minim, Marquee and Lantern, merged into one app whose face switches at runtime | [codemodify/media-player-music](https://github.com/codemodify/media-player-music) | `internal/players/…`, `examples/minim`, `examples/marquee`, `examples/lantern` |

What stayed is everything of the toolkit's the two of them used: all eight
**skins** in `style/skins` (the Theme Atlas still counts 131 packs), the
`skingen` generator including the plans named after the players, `rack`, and
a representative selection of their screenshots, each now labelled as an
application built with uitoolkit and linked to its repository. The two
applications are the proof the public API is enough to write a real desktop
application with, which is worth more than having them in the tree.
`docs/players.md` survives as a *toolkit* document — skins, slots, shaped
windows, snapping, and the toolkit changes writing them forced;
`docs/mail.md` is a stub that points at the other repo, because the
...
The theme list holds **131 packs**: 123 drawn by 31 engines and 8 skins
(Nocturne, Cassette, Deck, and the music player's Minim, Minim Classic,
Minim Silver, Marquee and Lantern), all generated by `cmd/uitk-skingen` so the
...
**What to try:** `examples/uitoolkit-sample-tour` first, then Settings (`cmd/uitoolkit-settings`),
Files (title-bar tabs, drag out), Inspector (docking), `examples/uitoolkit-sample-skinshape`
(a shaped window over a test card). The two applications above are the
end-to-end proof; they are run from their own repositories.
...
the on-desktop verification list, the GitHub PR refs and other
housekeeping, the Windows and macOS backends, IME and right-to-left text,
printing. (Mail's full-fledged rewrite went with Mail.)
...
painting and layout but never add behaviour; the player is visual only,
with our own artwork, and decodes nothing; commits carry the author alone; never run tests or apps
...
| kde1, kde2 | KDE 1 (Qt 1's Windows 95 widgets under kwm's window) and KDE 2's HighColor |
...
123 packs from 31 engines in all, with `web` and the later eras (above). Every agent branch is merged, the last
