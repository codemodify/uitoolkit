...
func TestAppIsAccessible(t *testing.T) {
	...
	w, err := a.NewWindow(platform.WindowOptions{Title: "notes", Width: 1040, Height: 700, Headless: true})
	if err != nil {
		t.Fatal(err)
	}
	defer w.Close()
	w.SetContent(NotesApp(w))
	a.PumpOnce()
	a11ytest.Audit(t, "notes", w.AccessibleTree())
}
