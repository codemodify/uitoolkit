#!/bin/bash
# theme-tour.sh N [PACK...] — show the tour's Controls page (the widget
# showcase) in every theme pack (or the
...
for P in "${PACKS[@]}"; do
  ...
  UITK_ANIMATIONS=0 UITK_THEME="$P" RUN_WAIT=3 "$RIG/run.sh" "$N" "$BIN/tour" -page controls >/dev/null
  "$RIG/shot.sh" "$N" "tour-$P" >/dev/null && mv "$D/shots/tour-$P.png" "$OUT/$P-gpu.png"
  # The same content on the CPU, headless (never on a display).
  TMP="$(mktemp -d)"
  (cd "$TMP" && env -u WAYLAND_DISPLAY -u DISPLAY UITK_ANIMATIONS=0 UITK_THEME="$P" XDG_CONFIG_HOME="$TMP/cfg" "$BIN/tour" -page controls -headless >/dev/null 2>&1 && mv tour.png "$OUT/$P-cpu.png")
  rm -rf "$TMP"
