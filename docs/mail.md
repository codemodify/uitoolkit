...
- **Folder tree** — account folders, Filters, and Outbox. Unified Folders, Smart Folders, Categories, and VIP are not shown. Click an account root to open that Inbox. The Filters group holds Unread / Starred / Attachment / From / To / Subject / Body pins (✓ + bold when on) plus the existing tag folders.
- **Chrome** — no path/subtitle strip, no bottom status bar, no sidebar Account / Folders section headers, no identity or Tags ComboBox, and no active-filter banner (`Filter on · N shown` / `Clear filter`) above the thread list. The folder tree (including Filters) starts at the top of the sidebar. The main toolbar keeps Get / Write / Cards / Classic on the left. Tag / Archive / Junk / Delete live on a small toolbar immediately above the Topic / Who / When header; the Quick Filter field sits in that same row after Delete, right-aligned (flex spacer). Hide or show the field with View → Quick Filter Bar (or Edit → Find / Ctrl+F to show and focus). Filter pins are only on the Filters tree — there is no filter toolbar and no left-side Quick Filter visibility toggle. Reply and Forward are menu / keyboard only (not on the main toolbar). File → Add Account / Remove Account / Account Central still manage stores.
