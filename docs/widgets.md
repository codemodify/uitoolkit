...
`go run ./cmd/uitk-shots docs/screenshots`. Isolated thumbs live
...
(Settings: "OS open/save dialogs"), or `UITK_NATIVE_DIALOGS=1`,
...
Where that JSON goes is the dock package's business too, so no application
has to invent a path. `host.SaveLayoutFile("inspector")` writes it to
`$XDG_CONFIG_HOME/uitoolkit/inspector-dock.json` (`dock.LayoutFile` gives
the path, and `ValidLayoutName` says which names are allowed — a plain app
name, never a path, so nothing can be written over `look.json`), and
`host.LoadLayoutFile("inspector")` reads it back, reporting whether there
was one:
...
host.OnLayoutChanged = func() { _ = host.SaveLayoutFile("inspector") }
```
