...
not a rectangle), and the five the player demos wear — Minim and the two
...
| `go run ./examples/minim` | Compact player: a 275x116 strip whose silhouette is its skin's, with an equaliser and a playlist that snap to its edges and travel with it; Ctrl+K, the skin key or a right-click switches all three windows between three skins, two of them whole panels |
