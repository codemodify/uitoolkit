...
// IMAP/POP3+SMTP+OAuth with an on-disk cache. No config means an empty store
...
func main() {
	sock := mailapp.DefaultSocket()
	store, err := mailapp.OpenStore()
	...
	if err := mailapp.ListenAndServe(ctx, sock, store); err != nil {
